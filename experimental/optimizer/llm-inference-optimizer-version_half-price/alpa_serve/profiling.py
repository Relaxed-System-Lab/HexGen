...
def load_test_prof_result(name: str, stage_costs: list):
    """Load pre-defined profiling results for testing."""
    para_dict = {}
    max_c = 64
    # Loop over the possible values of c
    for c in range(1, max_c + 1):
        weight_mem = [(1 / c) * GB] * c  # Distribute the total memory equally among c stages
        act_mem = ["TODO"] * c  # Create a list with c "TODO" strings

        para_dict[ParallelConfig(1, 1, c)] = LatencyMemData(
            latency={1: stage_costs},  # Slice the stage_costs list to get the first c elements
            act_mem={1: act_mem},
            weight_mem=weight_mem
        )
    if name == "Llama-70b":
        return ProfilingResult("Llama-70b",
            para_dict,
            preprocess_cpu=0,
            postprocess_cpu=0,
        )

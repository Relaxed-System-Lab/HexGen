...
class PossoinWorkLoad(WorkLoad):
    def __init__(self, model_num: int, tot_arrival_rate: float, proportions: List[float], duration: float, 
        ...
        assert model_num == len(proportions) == len(SLOs) and round(sum(proportions)) == 1 and tot_arrival_rate > 0
...
if __name__ == "__main__":
    ...
    # model_num, tot_arrival_rate, porportions, duration, SLOs:
    skew_workload = generate_workload(3, 10, [1/3] * 3, 20, [0.01] * 3, "./workload/pure_pipeline_3_models_slo_1e-2")
